...
@section('content')
    ...
    @php($page_current = 'jirai_issues_show')
    @include('components.backgrounds-builder',['page_current'=> $page_current])

    @if(!theme_config('jirai.items.issues-show') && theme_config($page_current.'.row'))
        @include('components.components-builder',['page_current'=> $page_current])
    @else
        <div class="container" id="jirai">

            <div
                class="card mt-5 mb-5" @include('elements.string-aos', ['pageAos' => 'jirai', 'itemAos'=>'issues-show'])>
                <div class="card-header">
                    <img class="rounded float-start me-4" src="{{ $issue->user->getAvatar() }}"
                         alt="{{ $issue->user->name }}" height="55">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            <a class="btn btn-danger"
                               href="{{ route('jirai.issues.edit', ['issue' => $issue->id]) }}">{{ trans('jirai::messages.edit') }}</a>
                            @endhasJiraiIssueEditPermission

                            @hasJiraiIssueDeletePermission($issue->user_id)
                            <a class="btn btn-danger"
                               href="{{ route('jirai.issues.destroy', ['issue' => $issue->id]) }}"
            ...
            @foreach($issue->messages as $message)
                ...
                @if($message->referenced_jirai_changelog_id != null)
                    <div
                        class="card shadow-sm mb-3" @include('elements.string-aos', ['pageAos' => 'jirai', 'itemAos'=>'messages-show'])>
                        <div class="card-header d-flex justify-content-between">
                            <small style="color: grey">
                                @lang('messages.posts.posted', ['user' => '<span class="btn btn-warning btn-sm me-3 rounded-pill">Auto</span>', 'date' => format_date($message->created_at, true)])
                            </small>
                        </div>
                ...
                @else
                    <div
                        ...
                        <div class="card-header d-flex justify-content-between">
                            <div class="d-flex ">
                                <img class="d-flex me-2 rounded" src="{{ $message->user->getAvatar() }}"
                                     alt="{{ $message->user->name }}" height="35">
                            ...
                            <div>
                                <div class="d-flex">
                                    ...
                                    <a class="btn btn-danger me-1"
                                       href="{{ route('jirai.messages.edit', ['message' => $message->id]) }}">{{ trans('jirai::messages.edit') }}</a>
                                    @endhasJiraiMessageDeletePermission($message->user_id)

                                    @hasJiraiMessageEditPermission($message->user_id)
                                    <a class="btn btn-danger "
                                       href="{{ route('jirai.messages.destroy', ['message' => $message->id]) }}"
                                       data-confirm="delete">{{ trans('jirai::messages.delete') }}</a>
            ...
            @can('jirai.message.post')
                <form action="{{ route('jirai.messages.store') }}"
                      method="POST" @include('elements.string-aos', ['pageAos' => 'jirai', 'itemAos'=>'messages-create'])>
                    @csrf
                    <div class="card shadow-sm mb-3">
                        <div class="card-header">
                            <span>{{ trans('jirai::messages.answer') }}</span>
                            <button type="submit"
                                    class="btn btn-primary float-end">{{ trans('jirai::messages.answer') }}</button>
                        </div>
