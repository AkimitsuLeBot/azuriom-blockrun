<li class="nav-item dropdown notifications-dropdown">
    ...
    <div class="dropdown-list dropdown-menu
        {{theme_config('header.navbar.style') == 1 ? 'dropdown-menu-end' : ''}}" aria-labelledby="notificationsDropdown">
        <h6 class="dropdown-header">{{ trans('messages.notifications.notifications') }}</h6>

        @if(! $notifications->isEmpty())
            <div id="notifications">
                @foreach($notifications as $notification)
                    <a href="{{ $notification->link ? url($notification->link) : '#' }}" class="dropdown-item d-flex media align-items-center">
                        <div class="me-3">
        ...
        <div id="noNotificationsLabel" class="dropdown-item text-center small text-success @if(! $notifications->isEmpty()) d-none @endif">
            <i class="bi bi-check"></i> {{ trans('messages.notifications.empty') }}
        </div>
    </div>
</li>
