        ...
        <div class="row g-0 {{$reverse}}">
            @if(theme_config($getItem.'.media'))
                <div class="col-md-{{$sizeL}}">
                    <img src="{{image_url(theme_config($getItem.'.media'))}}" class="img-fluid rounded-start {{theme_config($getItem.'.contain') ? '' : 'h-100'}}"
                         style="object-fit: {{theme_config($getItem.'.contain') ? 'contain' : 'cover'}};"
