<div class="card-header">
    <h3 class="m-0 fw--bold text-primary">{{ trans('theme::lang.articles.title') }}</h3>
</div>
<div class="card-body">
    @foreach($multiple_page as $k => $v)
        <div id="collapse-{{$page_current}}-{{$k}}" class="accordion-collapse collapse @if($loop->index == 0) show @endif"
            ...
            @if($k === 'index')
                @includeIf('admin.pattern.components-builder',['page'=> 'articles_index' ])
            @endif
            @if($k === 'show')
                @includeIf('admin.pattern.components-builder',['page'=> 'article_show' ])
            @endif
        </div>
    @endforeach
</div>
